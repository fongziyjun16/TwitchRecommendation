...
const renderCardTitle = (item, loggedIn, favs = [], favOnChange) => {
    ...
    return (
        <>
            {
                loggedIn &&
                <Tooltip title={isFav ? "Remove from favorite list" : "Add to favorite"}>
                    <Button
                        shape="circle"
...
function Home({resources, loggedIn, favoriteItems, favoriteOnChange}) {
    ...
    return (
        <Tabs defaultActiveKey={tabKeys.Streams}>
            <TabPane tab="Steams" key={tabKeys.Streams} forceRender={true}>
